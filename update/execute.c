...
int is_exec_other(char *path)
{
    ...
    // Eğer directory ise executable değil
...
char *ft_readline_loop(t_reader *r)
{
    ...
    // read() interrupted by signal veya eof kullaanay
    if (n <= 0 && r->pos == 0)
	{
        gc_free(r->buf);
        return NULL;
...
{
    // Heredoc signal handler'ı kurulumu
    heredoc_signals();
    ...
    while (1) 
    {
        ...
        if (!buf->line)
        {
            // Ctrl+D durumu veya signal  signal resetle lazım
        ...
        if (!heredoc_append_line(buf)) 
        {
            ...
            gc_free(buf->line);
            gc_free(buf->content);
            setup_interactive_signals(); // Signal resetle okey
            return NULL;
        }
        gc_free(buf->line);
        buf->line = NULL;
    ...
    // Heredoc bitince interactive signalse return aq
    setup_interactive_signals();
    return buf->content;
}
                          ...
                          t_env *env_list, int exit_status)
{
    ...
    while (1) 
    {
        ...
        if (!buf->line)
        {
            // Ctrl+D durumunda veya signal durumunda signali resetliyorum 
            setup_interactive_signals();
            break;
        }
        if ((size_t)ft_strlen(buf->line) == buf->delimiter_len && 
        ...
        if (!heredoc_append_expanded(buf, env_list, exit_status)) 
        {
            ...
            gc_free(buf->line);
            gc_free(buf->content);
            setup_interactive_signals(); // Signal resetle
            return NULL;
        }
        gc_free(buf->line);
        buf->line = NULL;
    ...
    // Heredoc bitince interactive signale geri dönüyorum
    setup_interactive_signals();
    return buf->content;
}
char *read_single_heredoc_block(char *delimiter)
{
    ...
    // readline_loop zaten signal durumunda NULL döndürür
    if (result)
        return result;
    else
        return buf.content;
...
char *read_heredoc_with_expand(char *delimiter, t_env *env_list, int exit_status)
{
    ...
    // Signal durumunda readline_loop_expand NULL döndürür zaten
    if (result)
        return result;
    else
        return buf.content;
...
{
	// Child process için signal'leri default'a çevir
	setup_child_signals();
...
int heredoc_handle(t_parser *cmd_list,t_exec_data *data, t_env *env_list, int exit_status)
{
	...
	while (current_cmd) 
	{
		...
		{
			// Heredoc başarısız - signal veya diğer hata
...
int execute_cmds(t_parser *cmd_list, char **env, t_env **env_list)
{
    ...
    if (heredoc_handle(cmd_list, &data, *env_list, data.last_exit_status))
        return (data.last_exit_status); // Signal durumunda 130 döndür
    ...
    // Parent process için signal ignore
    setup_parent_execution_signals();
    
    if (ft_data_pids(cmd_list, &data, env_list))
    {
        // Hata durumunda interactive signals'e geri dön
        setup_interactive_signals();
        return (1);
    }
    
    // İşlem sonunda interactive signals'e geri dön
    setup_interactive_signals();
